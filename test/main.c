...
int main()
{
    ...
    char *test_type1 = create_mq("1", 8);
    if (test_type1 == NULL) {
        printf("Failed to create '1' queue\n");
        return 1;
    }
    int test_qid1 = find_by_msg_type(test_type1)->queue_id;

    // Create second queue "2"
    char *test_type2 = create_mq("2", 8);
    if (test_type2 == NULL) {
        printf("Failed to create '2' queue\n");
        return 1;
    }
    int test_qid2 = find_by_msg_type(test_type2)->queue_id;
